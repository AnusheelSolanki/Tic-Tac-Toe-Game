...
public class GameFragment extends Fragment {
  ...
  private static final String GAME_UID = "gameUID";
  private static final String PLAYER_UID = "playerUID";
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    requireActivity().getTheme().resolveAttribute(com.google.android.material.R.attr.colorPrimary, typedValue1, true);
    activeColour = ContextCompat.getColor(requireContext(), typedValue1.resourceId);

    TypedValue typedValue2 = new TypedValue();
    requireActivity().getTheme().resolveAttribute(com.google.android.material.R.attr.colorOnBackground, typedValue2, true);
...
//        mLogic = savedInstanceState.getParcelable("logic");
//        Log.i("1-player","restored game from saved instance");
////        gameRef.child(mLogic.gameUUID).setValue(mLogic);
//      }
//      else {
...
//      if(savedInstanceState != null) {
//        mLogic = savedInstanceState.getParcelable("logic");
//        Log.i("2-player","restored game from saved instance");
////        gameRef.child(mLogic.gameUUID).setValue(mLogic);
//      }
//      else {
      ...
      public void handleOnBackPressed() {
        ...
        AlertDialog dialog = new AlertDialog.Builder(requireActivity())
                ...
                .setPositiveButton(R.string.yes, (d, which) -> {
                  ...
                  //endGameAsQuit();
                  updateDbEntryOnGameQuit(currentUser.getUid());
...
//  }
    ...
    gameRef.child(mLogic.gameUUID).addValueEventListener(new ValueEventListener() {
      ...
      public void onDataChange(@NonNull DataSnapshot snapshot) {
        ...
        if(mLogic.status.equals(getString(R.string.status_finished))){
          ...
          gameRef.child(mLogic.gameUUID).removeValue();
        }
      }
  ...
  private void updateDbEntryOnGameQuit(String playerUUID) {
    mLogic.status = getString(R.string.status_finished);
    mLogic.winner =  playerUUID.equals(mLogic.player1) ? "Player2" : "Player1";
    Game newObj = new Game(mLogic.open, mLogic.singlePlayer, mLogic.player1, mLogic.player2, mLogic.status, mLogic.winner, mLogic.turn,mLogic.tictactoe);
    gameRef.child(mLogic.gameUUID).setValue(newObj);
  }
  ...
  private void showDialogAndClose(String messageToDisplay, String titleToDisplay) {
    AlertDialog dialog = new AlertDialog.Builder(requireActivity())
            ...
            .setPositiveButton("OK", (d, which) -> {
              // TODO update loss count
              if(!back) mNavController.popBackStack();
            })
            .create();
    dialog.show();
  }
  ...
  public void onSaveInstanceState(Bundle outState) {
    outState.putString(GAME_UID, gameUUID);
    outState.putParcelable("logic", mLogic);
